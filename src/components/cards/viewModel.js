define([
  ...
  function CardsViewModel() {
    ...
    self.authView     = ko.observable('methods'); // 'methods' | 'mpin'
    ...
    self.authPick = function (method) {
      if (method === 'mpin') {
        ...
        // Biometric / OTP: simulate success with loader
        window.amanApp && window.amanApp.showLoader('Authenticating', 'Verifying your identity…');
        setTimeout(function () {
    ...
    self.mpinKey = function (k) {
      if (k === '⌫') { self.mpinVal(self.mpinVal().slice(0, -1)); return; }
      if (k === '' || self.mpinVal().length >= 6) return;
    ...
    self.renderCardInner = function (card) {
      ...
      return '<div style="position:absolute;inset:0;background:' + card.gradient + ';border-radius:18px;"></div>'
        ...
        +       '<span style="font-size:9px;font-weight:800;color:' + card.accentColor + ';letter-spacing:.5px;">AMAN BANK</span>'
        +     '</div>'
        +     '<div style="display:flex;align-items:center;gap:6px;">'
    ...
    self.toggleFreeze = function () {
      ...
      window.amanApp && window.amanApp.showToast(self.isFrozen() ? 'Card frozen' : 'Card unfrozen', 'success');
    };

    self.confirmBlock = function () {
      window.amanApp && window.amanApp.showToast('Card blocked successfully', 'success');
      self.closeBlockSheet();
    };

    self.saveLimits = function () {
      window.amanApp && window.amanApp.showToast('Limits updated', 'success');
      self.closeLimitSheet();
    };

    self.pinKey = function (k) {
      if (k === '⌫') { self.pinVal(self.pinVal().slice(0, -1)); return; }
